...
const ProductList: React.FC = () => {
  ...
  const categories = [
    ...
    { id: 'Residential Air Conditioning', name: 'RESIDENTIAL' },
    { id: 'Commercial Air Conditioning', name: 'COMMERCIAL' },
    { id: 'Industrial Cooling Solutions', name: 'INDUSTRIAL COOLING' },
    { id: 'Indoor Air Quality Solutions', name: 'IAQ SOLUTIONS' },
    { id: 'Controls & Automation', name: 'AUTOMATION' },
  ];
  ...
  return (
    <div className="bg-[#0a0a0a] min-h-screen text-white selection:bg-[#d4af37] selection:text-[#0a0a0a]">
        ...
        <div className="max-w-[85%] mx-auto text-left">
          <div className="space-y-4 max-w-4xl">
            ...
            <p className="text-xl text-gray-500 font-light leading-relaxed">
              Inovasi tata udara mutakhir untuk masa depan yang lebih hijau dan efisien.
            </p>
          </div>
        </div>
      </section>

      {/* Static Filters - Not Sticky */}
      <section className="bg-[#0a0a0a] py-8 border-b border-[#d4af37]/10 overflow-x-auto no-scrollbar">
        <div className="max-w-[85%] mx-auto px-8 flex flex-nowrap md:flex-wrap items-center gap-4">
           {categories.map(cat => (
             <button 
               key={cat.id}
               ...
               className={`text-[10px] font-bold tracking-[0.3em] uppercase transition-all px-6 py-2 rounded-full whitespace-nowrap border ${
                 activeTab === cat.id ? 'bg-[#d4af37] text-[#0a0a0a] border-[#d4af37]' : 'text-gray-500 hover:text-white border-white/5'
               }`}
             >
               {cat.name}
             </button>
      ...
      <section className="py-24 px-8">
        <div className="max-w-[85%] mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
            ...
            <div key={p.id} className="group bg-[#111] border border-white/5 rounded-[40px] p-8 hover:border-[#d4af37]/30 transition-all flex flex-col h-full text-left">
              ...
              <div className="space-y-4 flex-grow">
                ...
                <div className="flex flex-wrap gap-2 pt-2">
                  {p.features.map(f => <span key={f} className="text-[9px] font-bold text-gray-500 bg-white/5 px-3 py-1 rounded-full uppercase tracking-tighter">{f}</span>)}
                </div>
              </div>
